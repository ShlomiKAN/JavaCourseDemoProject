...
@Service
public class SportsService extends ItemServiceImpl implements ItemService {

    public SportsService(ItemRepository itemRepository) {
    ...
    @Override
    public void updateItem(Item item) throws InvalidOperationException {
        if(! item.getItemType().equals(ItemType.SPORTS))
            throw new InvalidOperationException("cannot update an item outside your domain");
        itemRepository.findById(item.getId()).orElseThrow(() -> new InvalidOperationException("cannot update not existing id"));
    ...
    public List<Item> getAllItems() {
        return itemRepository.findByItemType(ItemType.SPORTS);
    }

    @Override
    public int getNumberOfAllItems() {
        return itemRepository.countItemByType(ItemType.SPORTS.name());
    }
}
